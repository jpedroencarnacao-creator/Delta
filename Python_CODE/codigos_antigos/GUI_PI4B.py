...
if __name__ == '__main__':
    ...
    # --------------------------------------------------------------------
    # No Raspberry Pi, o WebKitGTK (usado por native=True/pywebview) tem
    # um bug de renderização conhecido (riscas) e fica lento ao corrigi-lo
    # via WEBKIT_DISABLE_DMABUF_RENDERER. Por isso aqui corre-se em modo
    # SERVIDOR (native=False) e abre-se o Chromium em modo kiosk, que tem
    # suporte de GPU muito mais maduro no Pi e fecha normalmente com
    # Alt+F4 ou Ctrl+W, sem ficar preso em ecrã cheio sem saída.
    #
    # A página web continua disponível para outros dispositivos na mesma
    # rede em http://<IP_do_Pi>:8081 (ex: telemóvel via QR code).
    # --------------------------------------------------------------------
    def _launch_kiosk_browser():
        import subprocess
        import time as _time
        _time.sleep(1.5)  # dá tempo ao servidor NiceGUI arrancar primeiro
        for browser_cmd in ('chromium-browser', 'chromium'):
            try:
                ...
                continue
        print('PI4B: Chromium não encontrado — abre manualmente http://localhost:8081 no browser.')

    threading.Thread(target=_launch_kiosk_browser, daemon=True, name='kiosk-launcher').start()
